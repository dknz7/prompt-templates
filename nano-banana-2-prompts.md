# Nano Banana 2 — Ad Creative Prompt Template Document

> **Credit:** Original Master Brand Research Claude prompts by Will Sartorius.

---

## Step-by-Step Process

1. Open a chat in Claude and enter the **Master Brand Research Prompt** (Section 1 below)
2. Download product images
3. Enter the **Claude Follow-up Prompt** (Section 2) and **Template Prompts** (Section 3) into Claude — attach the product images
4. Pick the templates you want to generate and enter them into **Higgsfield**

That's literally it.

---
## Section 1: Master Brand Research Prompt

**INPUT VARIABLES:**
- Target Brand Name: `[BRAND NAME]`
- Target URL: `[BRAND URL]`
...
## Section 2: Claude Follow-up Prompt
...
> **Note on example images:** The original Notion document contains example ad creative images for each template. To retain these for reference, save them to a local folder (e.g. `./examples/`) and reference them using HTML image tags in this file: `<img src="./examples/01-headline.jpg" alt="Headline example" width="400">` — see the note at the bottom of this document for full guidance.
...
```
Use the attached images as brand reference. Match the exact product design and colors precisely. Create: a static ad on a clean white background. Top: oversized bold black sans-serif headline reading "[HOOK HEADLINE like IF YOU'RE GOING THROUGH PERI..]" with [EMOJI] at the end. Center: a social media comment card with light gray rounded-rectangle background containing: a small circular profile avatar (top left), bold name "[REVIEWER NAME like Elaine McLean]", and a multi-sentence review in regular-weight sans-serif: "[FULL REVIEW TEXT, 3-4 sentences, conversational and emotional]". Small gray timestamp "[TIMESTAMP like 2d]" below the comment. Bottom center: [YOUR PRODUCT like product box and bar/bottle] photographed at a slight angle on white, soft studio lighting. No brand logo. No stars. The rawness is the point — this should look like someone screenshotted a real comment and dropped the product photo below it. 1:1 aspect ratio.
```
...
## Note: Retaining Example Images in Markdown

Markdown itself does not support embedding images from web URLs in a way that guarantees persistence, but you can **save example images locally** and reference them using an HTML `<img>` tag directly inside your `.md` file — most Markdown renderers (VS Code, Obsidian, Typora, GitHub, etc.) fully support inline HTML.

### Recommended folder structure

```
nanob2-prompts/
  nano-banana-2-prompts.md       ← this file
  examples/
    01-headline.jpg
    02-offer-promotion.jpg
    03-testimonials.jpg
    ...and so on for each template
```

### Inserting an example image into this file

Place this snippet directly below the relevant template section, replacing the filename and caption as needed:

```html
<img src="./examples/01-headline.jpg" alt="Headline template example" width="480">
```

You can also add a caption beneath it with standard Markdown italics:

```html
<img src="./examples/01-headline.jpg" alt="Headline template example" width="480">

*Example: Headline template — "Your Beard Deserves Better" (Sea Salt Beard Butter)*
```

### Tips

- **Width:** Set `width="400"` to `width="600"` depending on how wide you want the preview. Height scales automatically.
- **Obsidian users:** Works natively. You can also use Obsidian's own `![[filename.jpg]]` embed syntax if you keep the images inside your vault.
- **GitHub / GitLab:** Standard HTML `<img>` tags render correctly in `.md` files on both platforms.
- **VS Code:** Install the "Markdown Preview Enhanced" extension for full HTML image rendering in the preview pane.
- **Typora:** Renders HTML images inline by default.

> **To save the example images from the Notion source:** Right-click each example ad image on the original Notion page and choose "Save image as..." naming them to match the template number (e.g. `01-headline.jpg`, `02-offer-promotion.jpg`, etc.).
